...
# --- FONCTIONS UTILITAIRES & MOTEUR JSON ---
# Ces fonctions servent à la connexion et à la génération automatique des rapports.
...
def generate_status_json():
    """Moteur interne : Génère les données du rapport."""
    response = {
    ...
    if conn:
        try:
            ...
            metrics = ['Uptime', 'Threads_connected', 'Questions', 'Bytes_received', 'Bytes_sent']
            stats = {}
            for metric in metrics:
                cursor.execute(f"SHOW GLOBAL STATUS LIKE '{metric}'")
                row = cursor.fetchone()
                if row: stats[metric] = row[1]
            response["status"] = "UP"
...
def save_json_report(json_str):
    """Moteur interne : Sauvegarde le fichier."""
    try:
...
def generate_and_save_json_report():
    """Déclencheur automatique appelé après les exports/backups."""
    json_data = generate_status_json()
    fichier = save_json_report(json_data)
    if fichier:
        print(f"Diagnostic automatique généré : {fichier}")
    else:
        print("Échec du diagnostic automatique.")

# --- 1. FONCTIONNALITÉ EXPORT CSV ---
